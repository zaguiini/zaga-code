 ...
 */
export function fuzzyFileSearchTool(projectFiles: string[]) {
    const fuzzySearchSchema = z.object({
        query: z.string().describe("The search query to find files. Can be a partial filename, path segment, or loosely related term."),
    ...
    return tool(
        async (input: FuzzySearchInput) => {
            try {
                ...
                const countText = results.length === limit && results.length < projectFiles.length

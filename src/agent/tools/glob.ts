...
export function globTool(projectPath: string) {
    const globSchema = z.object({
        pattern: z.string().describe("The glob pattern to match files (e.g., '*.ts', '**\/*.ts', 'src/**\/*.tsx', or a specific file path)"),
    ...
    return tool(
        async (input: GlobInput) => {
            try {
                ...
                const fileCountText = fileCount === maxFiles && files.length > maxFiles

...
import { createExploreExecutorNode, exploreToolsCondition } from '@/nodes/explore'
import {
  createVerifyCleanupNode,
  createVerifyExecutorNode,
  createVerifySetupNode,
  verifyToolsCondition,
} from '@/nodes/verify'
...
async function buildAgentGraph(maxTokens: number) {
  ...
  return (
    new StateGraph(agentStateSchema)
      ...
      // Verify loop: executor → tools → executor (retries on empty, capped)
      .addConditionalEdges('verify-executor', verifyToolsCondition)

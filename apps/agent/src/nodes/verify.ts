...
export function createVerifyExecutorNode(
  ...
  return async (
    ...
    // Collect verify-phase continuation messages, filtering out empty AI responses
    const phaseMessages = state.messages.filter(m => {
      if (m.type === 'tool') return true
      if (m.additional_kwargs.phase !== 'verify') return false
...
/** Conditional edge for the verify loop — retries on empty/malformed responses. */
export function verifyToolsCondition(
  state: AgentState
): 'verify-tools' | 'verify-executor' | 'verify-cleanup' {
  const lastMessage = state.messages[state.messages.length - 1]

  if (lastMessage.type !== 'ai') return 'verify-cleanup'

  const verifyToolResults = state.messages.filter(
    m => m.type === 'tool' && state.messages.some(a => a.additional_kwargs.phase === 'verify')
  ).length
  if (verifyToolResults >= MAX_VERIFY_ITERATIONS) return 'verify-cleanup'

  const rootToolCalls = (lastMessage as { tool_calls?: Array<unknown> }).tool_calls
  if (Array.isArray(rootToolCalls) && rootToolCalls.length > 0) return 'verify-tools'

  const content = String(lastMessage.content).trim()
  if (content.length > 0) return 'verify-cleanup'

  // Count consecutive empty verify responses at the end of messages
  let emptyCount = 0
  for (let i = state.messages.length - 1; i >= 0; i--) {
    const m = state.messages[i]
    if (m.type !== 'ai' || m.additional_kwargs.phase !== 'verify') break
    if (String(m.content).trim() === '') emptyCount++
    else break
  }

  if (emptyCount >= MAX_EMPTY_RETRIES) return 'verify-cleanup'

...
export function grepTool(projectPath: string) {
  const grepSchema = z.object({
    ...
    filePattern: z
      ...
      .optional()
      .describe(
        "Optional file pattern to search in (e.g., '*.ts', '**/*.ts', or a specific file path). If not provided, searches all files recursively."
      ),
    caseSensitive: z
      .boolean()
      .optional()
  ...
  return tool(
    async (input: GrepInput) => {
      try {
        ...
        // Format results
        const formattedResults = results.map(r => `${r.file}:${r.line}: ${r.content}`).join('\n')
    ...
    },
    {
      name: 'grep',
      description:
        'Search for text patterns in files within the project directory. Supports regex patterns and file glob patterns. Returns matching lines with file paths and line numbers.',
      schema: grepSchema,
    }
  )
}
